...
async def test_mux(dut):
       ...
       for n in range (32):
              ...
              num = ran_val
              # Storing all these random values along with the mux_inp# in a list
              mux_inr.append(ran_val)
              mux_inp_all.append(num)

       # Sending inputs to DUT
       ...
       # Monitoring output based on selection cases
       for s_val in range (31):
              print (f"Test Case #{s_val}")
              dut.sel.value = s_val

              await Timer(2, units="ns")
              # Display of input/output values
              print (f"inp{s_val}={bin(mux_inp_all[s_val])}\tsel={s_val}\tout={dut.out.value}")
